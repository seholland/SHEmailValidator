...
- (BOOL)validateSyntaxOfEmailAddress:(NSString *)emailAddress withParts:(NSArray *)emailParts withError:(NSError **)error
{
	if (emailAddress.length == 0) {
		...
		NSError *newError = [NSError errorWithDomain:SHValidatorErrorDomain code:SHBlankAddressError userInfo:errorDictionary];
		if (newError)
		{
			*error = newError;
		}
	...
	if (![fullEmailPredicate evaluateWithObject:emailAddress]) {
		...
		NSError *newError = [NSError errorWithDomain:SHValidatorErrorDomain code:SHInvalidSyntaxError userInfo:errorDictionary];
		if (newError)
		{
			*error = newError;
		}
	...
	if (username.length == 0 || ![usernamePredicate evaluateWithObject:username] || [username hasPrefix:@"."] || [username hasSuffix:@"."] || ([username rangeOfString:@".."].location != NSNotFound)) {
		...
		NSError *newError = [NSError errorWithDomain:SHValidatorErrorDomain code:SHInvalidUsernameError userInfo:errorDictionary];
		if (newError)
		{
			*error = newError;
		}
		...
		NSError *newError = [NSError errorWithDomain:SHValidatorErrorDomain code:SHInvalidDomainError userInfo:errorDictionary];
		if (newError)
		{
			*error = newError;
		}
		...
		NSError *newError = [NSError errorWithDomain:SHValidatorErrorDomain code:SHInvalidTLDError userInfo:errorDictionary];
		if (newError)
		{
			*error = newError;
		}
